...
function setupReglages() {
  ...
  $("#btn-gh-pull").addEventListener("click", async () => {
    ...
    try {
      ...
      if (!payload) {
        status.textContent =
          `Aucun fichier trouvé à « ${state.settings.github.path} » sur ${state.settings.github.repo || "(dépôt non renseigné)"} ` +
          `(branche ${state.settings.github.branch || "main"}). Vérifiez le dépôt/la branche, ou ce sera créé à la prochaine sauvegarde.`;
        return;
      }
      state.progress = mergeProgress(state.progress, payload);
      saveProgress(state.progress);
